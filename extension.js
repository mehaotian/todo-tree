...
// 允许文件类型
const fileWhitelist = ['vue', 'nvue', 'js', 'wxs', 'ts', 'html', 'css', 'scss', 'less', 'md', 'json']
...
async function showWebView(webview) {
	...
	let link = `<link rel="stylesheet" type="text/css" href="${path.join(__dirname,'style/index.css')}"></script>`
	let icons = `<link rel="stylesheet" type="text/css" href="${path.join(__dirname,'style/icons.css')}"></script>`
	let icons2 = `<link rel="stylesheet" type="text/css" href="${path.join(__dirname,'style/iconfont.css')}"></script>`
	if (isDev) {
...
<html lang="en">
  ...
  <body>
	...
	let onDidSaveTextDocumentEventDispose = hx.workspace.onDidSaveTextDocument(async (document) => {
		...
		if (!isDirty && !isUntitled && fileWhitelist.includes(fileExt)) {
			const folderPath = document.workspaceFolder.uri.path
			const id = path.relative(path.dirname(folderPath), path.join(fsPath, '..'));
...
function openSearch(event) {
	...
	const pickResult = hx.window.showQuickPick(list, { placeHolder: '请输入内容' });
